...
def find_equilibrium_points():
    ...
    # Solve the equations to get equilibrium points and store the values in equi_points.
    equi_points = sp.solve(equations, STATES)
...
def find_A_B_matrices(eq_points):
    ...
    # Jacobian matrices for each equilibrium point
    for point in eq_points:
    
            #Substitute equilibrium point values and zero input.
            substitutions = {
                theta: point[0],
                omega: point[1],
                u: 0
             ...
             # Storing the A and B matrices for each equilibrium point 
             # in A_matrices, B_matrices respectively 
            A_matrices.append(A.subs(substitutions))
            B_matrices.append(B.subs(u, 0))
...
def find_eigen_values(A_matrices):
    ...
    for A in A_matrices:
        ...
        # Compute the eigen value of matrix A as dictionary keys and store it in eigen_dict
        eigen_dict = A.eigenvals()

        # Store the computed eigen values as dictionary keys in eigen_values matrix.
        eigen_values.append(eigen_dict)

        # eigen_dict.keys() : extracts the eigenvalues from the dictionary that are stored as keys.
        # for val in eigen_dict.keys() : loop that iterates through every key in the dictionary and stores it in the variable 'val'
        # Check if the real part of the eigen value is negative
        if all(sp.re(val)<0 for val in eigen_dict.keys()):
